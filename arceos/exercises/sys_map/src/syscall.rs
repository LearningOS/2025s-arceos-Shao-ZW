...
use axhal::mem::{phys_to_virt, MemoryAddr, VirtAddr, PAGE_SIZE_4K};
...
fn sys_mmap(
    ...
    if addr.is_null() {
        if let Some(free_vaddr) = uspace.find_free_area(
            vaddr,
            align_up_4k(length),
    ...
    uspace.map_alloc(
        vaddr,
        align_up_4k(length),
        MappingFlags::from(MmapProt::from_bits(prot).unwrap()),
        true,
    );
